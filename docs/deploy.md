...
Домен проекта — **race.live** (ADR-009). Настройка: A-запись `race.live` → IP VPS (и `www`
при желании), reverse-proxy (Caddy/Nginx) терминирует TLS. В `.env` прода выставить
`NEXT_PUBLIC_API_URL=https://race.live` (или поддомен API, напр. `https://api.race.live`).
До готовности прод-домена можно поднять на поддомене для проверки — смена домена позже это
правка конфига прокси и `NEXT_PUBLIC_API_URL`, не переезд.
