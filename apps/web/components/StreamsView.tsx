...
function RecBadge() {
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-mute"
      style={{ background: "var(--surface-2)" }}
      title="Кастер не в эфире — показана его последняя запись"
    >
      Запись
    </span>
  );
...
export function StreamsView({ streams }: { streams: StreamOut[] }) {
  // Приоритет выбора: живой эфир → кастер с записью → первый в списке.
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* СТАТУС: есть эфир / только записи / ничего */}
        ...
        <div className="flex min-w-0 flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2.5">
              {current.live ? <LiveBadge /> : current.embed_url ? <RecBadge /> : null}
